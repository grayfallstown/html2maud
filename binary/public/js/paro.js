...
(function() {
    ...
    function getCssPath(el) {
        ...
        var break_on_id = true;
        var path = [];
        while (el.nodeType === Node.ELEMENT_NODE) {
        //while (el) {
            var selector = el.nodeName.toLowerCase();
            if (break_on_id && el.id) {
                selector += '#' + el.id;
     ...
     */
    PARO.emitEvent = (event_id, event) => {
        PARO = window.__PARO__;
        if (PARO.websocket) {
            if (PARO.logging)
                PARO.logger.info("[paro emit event] emitting event '" + event_id + "'", event);
            function get_value_and_emit() {
                var value = undefined;
                var target = event.target;
                var cssPath = getCssPath(target);
                var startPosition = target.selectionStart;
                var endPosition = target.selectionEnd;
                if (target) {
                    value = target.value;
                ...
                setTimeout(() => {
                    // once the server replied with new html, go back to the
                    // event emitting element, set focus and cursor position
